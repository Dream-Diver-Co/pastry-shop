...
function okay(event) {
  ...
  if (event.target.innerText == "continue") {
      ...
      event.target.parentElement.getElementsByClassName("order-details")[0].innerHTML = `<em class='thanks'>Thanks for shopping with us</em>`;
...
function QtyBtn(qty = 1) {
  ...
  return `
  <div class="qty-change" style="display: flex;">
      ...
      productDetails.forEach((product) => {
          if (item.name == product.name && product.qty >= 0) {
              ...
              if (product.qty < 0) {
                  ...
                  document.getElementsByClassName("order-details")[0].innerHTML = `<em class='thanks'>Stocks Limit Exceeded</em>`;
...
function OutOfStock(product, handler) {
  ...
  for (let items of products) {
      ...
      if (product.name == name) {
          if (handler) {
              items.getElementsByClassName("out-of-stock-cover")[0].style.display = "flex";
